...
function batchgetall_test(storeName, func, name) {
    indexeddb_test(
        (t, connection, tx) => {
            ...
            switch (storeName) {
                case "generated":
                    store = connection.createObjectStore("generated", {
                        autoIncrement: true,
                        keyPath: "id",
                    });
                ...
                case "out-of-line-not-unique":
                    ...
                    alphabet.forEach((letter) => {
                        ...
                        else store.put({ ch: letter, half: "third" }, letter);
                    });
                    break;
...
batchgetall_test(
    ...
    (t, connection) => {
        var req = createBatchGetAllRequest(
            ...
            "out-of-line",
            connection,
            [IDBKeyRange.bound("C", "M"), IDBKeyRange.bound("O", "Z")],
            3
        );
        req.onsuccess = t.step_func(function (evt) {
            let result = evt.target.result;
...
batchgetall_test(
    "out-of-line-multi",
    (t, connection) => {
        var req = createBatchGetAllRequest(t, "out-of-line-multi", connection, [
            "vowel",
        ]);
        req.onsuccess = t.step_func(function (evt) {
            let result = evt.target.result;
